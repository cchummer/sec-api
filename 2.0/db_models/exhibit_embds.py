...
class ExhibitEmbeddings(Base):
    __tablename__ = settings.EXHIBITS_EMBEDDINGS_TABLE

    id = Column(Integer, primary_key=True)
    # Relationships
    #filing_info = relationship(settings.FILING_INFO_CLASS_NAME, back_populates=settings.NAMED_SECTION_EMBEDDINGS_TABLE)
    exhibits = relationship(settings.EXHIBITS_CLASS_NAME, back_populates=settings.EXHIBITS_EMBEDDINGS_TABLE)
    ...
    # Foreign key to exhibits
    section_id = Column(Integer, 
                       ForeignKey(f'{settings.EXHIBITS_TABLE}.id'),
                       nullable=True,
                       index=True)
