...
class NamedSectionEmbeddings(Base):
    __tablename__ = settings.NAMED_SECTIONS_EMBEDDINGS_TABLE

    id = Column(Integer, primary_key=True)
    # Relationships
    #filing_info = relationship(settings.FILING_INFO_CLASS_NAME, back_populates=settings.NAMED_SECTION_EMBEDDINGS_TABLE)
    named_sections = relationship(settings.NAMED_SECTIONS_CLASS_NAME, back_populates=settings.NAMED_SECTIONS_EMBEDDINGS_TABLE)
    ...
    # Foreign key to named_sections 
    section_id = Column(Integer, 
                       ForeignKey(f'{settings.NAMED_SECTIONS_TABLE}.id'),
                       nullable=True,
                       index=True)
